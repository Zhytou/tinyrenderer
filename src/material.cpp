...
Material::Material(const std::filesystem::path& baseDir, const tinyobj::material_t& material) {
    ...
    if (!material.diffuse_texname.empty()) {
        m_textures["albedo"] = Texture::create(baseDir / material.diffuse_texname, GL_RGBA8);
    } else {
        m_textures["albedo"] = Texture::create({material.diffuse[0], material.diffuse[1], material.diffuse[2], 1.0f}, GL_RGBA8);
    }

    if (!material.normal_texname.empty()) {
        m_textures["normal"] = Texture::create(baseDir / material.normal_texname, GL_RGBA8);
    } else {
    ...
    if (!material.metallic_texname.empty() && !material.roughness_texname.empty() && !material.ambient_texname.empty()) {
        ...
        m_textures["mrao"] = Texture::create({material.metallic, material.roughness, 0.f, 0.f}, GL_RGBA8);
    }
}
